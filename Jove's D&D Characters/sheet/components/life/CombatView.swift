	...
	if c.life.combat.hasContent {
		Grid(cols: [.init(.fill()), .init()]) {
			...
			Grid(cols: columns, rows: .init(.uniform())) {
				c.life.combat[0...2].map { item in
					Panel(theme, aspectRatio: true) {
						Grid(vertFlow: .init(), rows: .init(align: .center)) {
							...
							JCSText(item.score?.signedDescription(apply: item.isBonus), font: theme.largeAttributeFont, color: theme.ink, lines: 1)
						}
					}
				}
				Panel(theme, aspectRatio: true) {
					Grid(vertFlow: .init(), rows: .init(align: .center)) {
						...
						JCSText(c.life.health.maxHitPoints?.description, font: theme.largeAttributeFont, color: theme.ink, lines: 1)
					}
				}
				Panel(theme, aspectRatio: true) {
					Grid(vertFlow: .init(), rows: .init(align: .center)) {
						JCSText(jargon.currentHitPointsTitle, font: theme.smallNoteBoldFont, color: theme.ink, lines: 2)
				...
				Panel(theme, aspectRatio: true) {
					Grid(vertFlow: .init(), rows: .init(align: .center)) {
						JCSText(jargon.temporaryHitPointsTitle, font: theme.smallNoteBoldFont, color: theme.ink, lines: 2)
						JCSText(c.life.health.temporaryHitPoints?.description, font: theme.largeAttributeFont, color: theme.ink, lines: 1)
					}
				}
				Panel(theme, aspectRatio: true) {
					Grid(vertFlow: .init(), rows: .init(align: .center)) {
						JCSText(jargon.deathSavesTitle, font: theme.smallNoteBoldFont, color: theme.ink, lines: 2)
						JCSText("\(jargon.deathSaveSuccessTitle) \(String(repeating: jargon.deathSaveMarker, count: 3))\n\(jargon.deathSaveFailureTitle)\(jargon.labelSeparator) \(String(repeating: jargon.deathSaveMarker, count: 3))", font: theme.smallNoteBoldFont, color: theme.ink, lines: 2)
					}
				}
			}
		}
