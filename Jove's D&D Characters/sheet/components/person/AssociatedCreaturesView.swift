			...
			c.person.associatedCreatures.filter { !$0.isEmpty }.map { creature in
				Panel(theme) {
					...
					Grid(vertFlow: .init(dimension)) {
						JCSText("\(creature.name) — \(creature.kind.description)", font: theme.featureHeadingFont, color: theme.ink)
						JCSText([
						...
						].compactMap { $0 }.filter { !$0.isEmpty }.joined(separator: " • "), font: theme.featureBodyFont, color: theme.ink)
						JCSText((creature.notes + (stats?.notes ?? [])).joined(separator: "; "), font: theme.smallNoteFont, color: theme.ink)
					}
				}
			}
		}
