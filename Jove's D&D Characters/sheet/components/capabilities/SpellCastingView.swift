			...
			c.capabilities.spellcasting.map { casting in
				Panel(theme) {
					Grid(vertFlow: .init(dimension)) {
						JCSText(casting.source.isEmpty ? "Spellcasting" : casting.source, theme, font: .lineItemBold, color: .ink)
						JCSText([
						...
						].compactMap { $0 }.joined(separator: " • "), theme, font: .body, color: .ink)
						JCSText(casting.slots.filter { !$0.isEmpty }.map { "\($0.level.description): \($0.counter.description)" }.joined(separator: " • "), theme, font: .lineItemBold, color: .ink)
						casting.spells.filter { !$0.isEmpty }.map { spell in
							JCSText("\(spell.isPrepared == true ? "● " : "")\(spell.name) — \(spell.level.description)\(spell.detail.isEmpty ? "" : ": \(spell.detail)")", theme, font: .body, color: .ink, lines: 0...3)
						}
						JCSText(casting.notes.joined(separator: "; "), theme, font: .body, color: .ink)
					}
				}
			}
		}
